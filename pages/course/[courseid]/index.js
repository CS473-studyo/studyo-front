...
const Course = () => {
  ...
  const Hashtag = props => {
    return (
      <div className="hashtag body-text">#{props.keyword}</div>
    )
  }
  ...
  const LectureElem = props => {
    return (
      ...
      <Card.Body>
        ...
        <div className="row">
          <a href={`/course/${courseid}/${props.num}/note`} type="button" className="mt-4 custom-btn mr-3">Lecture Note</a>
          <a onclick={`/course/${courseid}/${props.num}/review`} type="button" className="mt-4 custom-btn">Review Quiz</a>
        </div>
      </Card.Body>
      </Card>
    );
